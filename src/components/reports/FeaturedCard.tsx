...
export function FeaturedCard({ article }: { article: PayloadArticle }) {
  ...
  return (
    <Link
      ...
      className="group relative block h-95 flex-[0_0_100%] snap-start overflow-hidden rounded-xl border border-zinc-300 transition-[box-shadow,transform] duration-200 hover:-translate-y-1.5 hover:shadow-[0_14px_44px_rgba(27,67,50,0.24)] sm:h-105 lg:flex-[0_0_calc(50%-8px)]"
    >
      ...
      <div className="absolute left-4 top-4 z-10 flex gap-1.5">
        ...
        <span className="rounded-full border border-white/20 bg-white/12 px-2.5 py-1 text-[11px] text-white/85">
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 p-5 sm:p-6">
        <h3 className="mb-2 font-display text-lg font-bold leading-[1.24] tracking-[-0.02em] text-white text-pretty sm:text-xl">
          {article.title}
        </h3>
        {article.summary ? (
        ...
        {stats.length > 0 ? (
          <div className="mb-3.5 grid grid-cols-3 border border-white/15 bg-white/10">
            {stats.map((stat, i) => (
              <div
                key={stat.label}
                className={`px-1.5 py-2.5 text-center sm:px-2 ${i === 1 ? "border-x border-white/15" : ""}`}
              >
                <div className="font-display text-lg font-bold leading-none text-white sm:text-xl">
                  {stat.value}
                </div>
