...
**Funding.** Verified live on 2026-07-21. Two tiers: one **structured** source, three **unstructured** RSS fallbacks.

| Priority | Source | Tier | Verified status |
| --- | --- | --- | --- |
| **1 (primary)** | **PredictLeads — Financing Events dataset** | Structured (`company`, `amount`, `round`, `investors`, `date` as real fields) | Free-forever tier: **100 API credits/month, $0**, not a trial. Billing is "1 request = 1 credit" for their event-style endpoints (Job Openings, News Events, Technology Detections all confirmed at up-to-1,000-records-per-credit); Financing Events likely follows the same pattern but this is **unconfirmed** — their docs site truncated before showing the exact rate. **Action before building**: sign up for a free key and fire one real request to settle this and capture the true response shape. |
| **2 (fallback)** | **TechCrunch** — `https://techcrunch.com/category/venture/feed/` | RSS, needs extraction | Live-tested, real hit: *"Natural raises $30M to reinvent payments for AI agents"* (2026-07-20). Purpose-built "Venture Capital News" category — highest signal-to-noise of the RSS options. |
| **3 (fallback)** | **VentureBeat AI** — `https://venturebeat.com/category/ai/feed` (no trailing slash; `.../feed/` 308-redirects here) | RSS, needs extraction | Live-tested. AI-specific but not funding-specific, so most items need to be discarded by keyword filter. |
| **4 (fallback, use cautiously)** | **Google News RSS** — `https://news.google.com/rss/search?q="AI+startup"+funding+when:1d&hl=en-US&gl=US&ceid=US:en` | RSS, needs extraction | Live-tested, widest net. Its own feed copyright notice restricts use to *"personal, non-commercial use... within a personal feed reader"* — fine for internal discovery/dedup, but check with legal before republishing its content verbatim on a commercial site. |

Dropped from the plan: **Crunchbase** (their news RSS is just more general VC journalism, redundant with #2/#3; their real structured funding API has no free tier — paid/enterprise only) and **PredictLeads' own paid tiers** (pay-as-you-go past the free 100 credits — revisit only if volume outgrows the free tier).

**Pipeline:**

1. **PredictLeads first**, gated by a monthly credit budget (see `ApiUsage` above) — skip the call once `creditsUsed >= creditsCap` for the current period and fall through to RSS-only for the rest of the month. Its response maps straight onto `FundingEventDTO` — no regex needed.
2. **RSS fetchers run unconditionally**, in parallel, regardless of whether PredictLeads succeeded — they're free and catch stories PredictLeads' source coverage misses. Filter each feed for AI + funding keywords, then extract `{ company, amount, round, investors }` from the headline/summary with regex (`/\$(\d+(?:\.\d+)?)\s*(M|B|million|billion)/i`, `/(Seed|Series [A-H]|Pre-Seed)/i`). Expect ~70% accuracy — acceptable in Phase 1 because a human writes the summary anyway. Phase 2 replaces this regex pass with an LLM extraction pass over the same stored `raw` payloads.
3. **Store every mention as its own row**, even when TechCrunch, VentureBeat, and PredictLeads all cover the same raise — raw data is never deduplicated away, only the *report* dedupes (next point), so provenance and cross-source comparison stay available.
4. **Resolve duplicates at report-build time, not ingestion time.** In `src/reports/funding-report.ts`, group the day's rows by `(companySlug, round, announcementDate ±3 days)`. If a PredictLeads row exists in a group, treat it as canonical for the report's numbers (structured > regex-guessed); otherwise merge the best available fields across the RSS rows in the group. This is what keeps "total funding volume" from triple-counting one raise covered by three outlets.
